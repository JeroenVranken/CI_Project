...
for filename in training_names:
    ...
    my_data = np.append(my_data, preprocessing.MinMaxScaler().fit_transform(genfromtxt(filename, delimiter=',')[1:]), axis=0)
print(my_data.shape)
...
def get_batch(sequence_size=10):
    """Builds a batch i.e. (x, f(x)) pair."""
    x = np.empty([sequence_size, len(my_data.T)])
    y = np.empty([1, 3])
    random_int = np.random.randint(0, len(my_data) - (sequence_size + 1))
    x = my_data[random_int:random_int + sequence_size]
    y = my_data[random_int + sequence_size + 1][0:3]
...
model = SimpleLSTM(D_in, sequence_size, hidden_size, D_out)
